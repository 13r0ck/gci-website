        ...
        earthVideo =
            html <|
                video
                    ...
                    []
    in
    row [ width fill, height (px 500), Background.color (rgb 0 0 0), clip ]
        [ el
            [ width fill
        ...
        testimonial i t =
            acol
                ...
                , column [ paddingEach { top = 24, bottom = 48, left = 48, right = 48 }, spacing 18, height fill ]
                    [ paragraph [ Font.medium ] [ text ("\"" ++ t.quote ++ "\"") ]
                    , paragraph [ Font.extraLight, Font.alignRight, Font.size 18, alignBottom ] [ text t.attribution ]
                    ]
                ]
    in
    column [ width fill ]
        [ row [ width fill, height shrink, spacing 48, paddingXY 48 0 ] (List.indexedMap testimonial ts)
        ]


glassLogo : Element msg
glassLogo =
    image [ width fill, behindContent blur ] { src = "/img/glass_logo5.png", description = "GCI logo on glass" }
